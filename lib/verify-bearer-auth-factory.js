...
/**
 * @description Factory function to create a verifyBearerAuth function with the provided options.
 * @type {import('../types/index').verifyBearerAuthFactory}
 */
module.exports = function verifyBearerAuthFactory (options) {
  ...
  /**
   * @description Handles unauthorized access by sending a 401 response and logging the error if configured.
   * @param {import('fastify').FastifyRequest} request - The incoming request object.
   * @param {import('fastify').FastifyReply} reply - The reply object to send the response.
   * @param {(err?: Error) => void} done - The callback function to signal completion.
  ...
  /** @type {import('../types/index').verifyBearerAuth} */
