...
module.exports = function verifyBearerAuthFactory (options) {
  ...
  const { keys, errorResponse, contentType, bearerType, bearerTypeCaseSensitive, auth, addHook = true, verifyErrorLogLevel = 'error' } = _options
  ...
  const bearerTypePrefix = bearerTypeCaseSensitive
    ? bearerType + ' '
    : bearerType.toLowerCase() + ' '

  const verifyBearerType = bearerTypeCaseSensitive
